...
export const AlumniPage: React.FC<AlumniPageProps> = ({ onEnroll }) => {
  ...
  const [activeTab, setActiveTab] = useState<'all' | 'engineering' | 'management' | 'non-tech'>('all');
  ...
  const parseQualification = (qual: string) => {
    ...
    // Look for IIT, NIT, ITER, VSSUT, HSBC, etc.
    ...
    // Fallback split for "Degree Specialization" or just single line
    return {
      degree: trimmed,
      institution: ''
    };
  ...
  // Categorize alumni based on educational qualifications
  const checkEducationCategory = (qualification: string): 'engineering' | 'management' | 'non-tech' => {
    ...
    if (
      ...
      q.includes('vssut') || 
      q.includes('eee') || 
      q.includes('applied electronics')
    ) {
      return 'engineering';
    }
    
    if (q.includes('mba') || q.includes('b.com') || q.includes('bcom') || q.includes('business strategy')) {
      return 'management';
    }
  ...
  const filteredAlumni = useMemo(() => {
    return alumniData.filter((alumnus) => {
      ...
      const matchesSearch = 
        ...
        alumnus.qualification.toLowerCase().includes(query);
      
      if (!matchesSearch) return false;

      if (activeTab === 'all') return true;
      const category = checkEducationCategory(alumnus.qualification);
  ...
  return (
    <div className="alumni-page-container">
      ...
      <section className="alumni-main-section container">
        ...
        <div className="alumni-controls-wrapper">
          <div className="alumni-search-box">
            ...
            <input 
              type="text" 
              placeholder="Search by name, company, or qualification..." 
              value={searchQuery}
          ...
          {/* Education background filter tabs */}
          <div className="alumni-filter-tabs">
            ...
            <button 
              type="button" 
              className={`filter-tab-btn ${activeTab === 'engineering' ? 'active' : ''}`}
            ...
            <button 
              type="button" 
              className={`filter-tab-btn ${activeTab === 'management' ? 'active' : ''}`}
            ...
            <button 
              type="button" 
              className={`filter-tab-btn ${activeTab === 'non-tech' ? 'active' : ''}`}
              onClick={() => setActiveTab('non-tech')}
            >
              Non-Tech / Science / Other
            </button>
          </div>
        </div>

        {/* Results Info */}
        <div className="alumni-results-info">
          Showing {filteredAlumni.length} alumni {activeTab !== 'all' ? `with ${activeTab === 'engineering' ? 'Engineering' : activeTab === 'management' ? 'Management' : 'Non-Tech'} background` : ''}
        </div>
        ...
        {filteredAlumni.length > 0 ? (
          ...
          <div className="alumni-no-results">
